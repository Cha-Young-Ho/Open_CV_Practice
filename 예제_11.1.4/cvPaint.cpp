...
int main()
{
    image = Mat(620, 800, CV_8UC3, Scalar(255, 255, 255));   // 그림판 영상
    ...
    while (1)
    {
        ...
        {
            draw(image, Color);
            if (draw_mode == DRAW_SPRAY) {
                pt1 = pt2 = { -10, -10 };
            }
        }
        else if (mouse_mode == 3)         // 마우스 드래그
        {
            if (draw_mode == DRAW_BRUSH || draw_mode == ERASE || draw_mode == DRAW_TEXT || draw_mode == DRAW_SPRAY) {
                draw(image, Color);
            }
